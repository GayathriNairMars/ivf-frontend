...
import { STATUS_COLORS,PATIENT_STATUSES } from "../../../../utils/constants";
...
export default function PatientProfile() {
	...
	const handleStatusChange = async (newStatus) => {
		setUpdating(true);
		try{
			await api.post(`/patients/${id}/update-status`,{status:newStatus});
					...
					{patient.partner_info? (
						<>
							...
							<button className="btn-secondary" style={{marginTop: 12, fontSize: "0.8rem"}} onClick={async () => {
								await api.post(`/patient/${id}/unlink-partner/`);
