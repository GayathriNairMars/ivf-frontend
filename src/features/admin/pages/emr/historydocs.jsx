...
export default function HistoryDocuments({patient}) {
	...
	const load =useCallback(async () => {
		setLoading(true);
		try {
			const {data} = await api.get(`/emr/patient/${patient.id}/history/`);
	...
	const handleSubmit = async e => {
		...
		try{
			...
			await api.post(`/emr/patient/${patient.id}/history/add`,payload,{
	...
	const handleDelete = async (docId) => {
		...
		try {
			await api.delete(`/emr/history/${docId}/delete/`);
			load();
