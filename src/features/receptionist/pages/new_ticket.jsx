...
export default function NewTicket({onSuccess,onCancel}) {
	...
	useEffect(() => {
		api.get("/receptionist/tickets/doctors/").then(({data}) => setDoctors(data)).catch(() => {});
		api.get("/receptionist/tickets/departments/").then(({data}) => setDepartments(data)).catch(() => {});
	},[]);
	...
	useEffect(() => {
		...
		const t = setTimeout(() => {
			api.get(`/receptionist/patients/?search=${search}`)
	...
	const handleSubmit = async (e) => {
		...
		setSubmitting(true);
		try {
			const {data} = await api.post("/receptionist/tickets/",{
				patient: selected.id,
