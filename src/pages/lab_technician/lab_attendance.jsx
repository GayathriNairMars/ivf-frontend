...
export default function LabAttendance() {
	...
	const fetchData = async () => {
		setLoading(true);
		try {
			const todayRes = await labApi.getTodayAttendance().catch(() => null);
			const histRes  = await labApi.getAttendanceHistory({}).catch(() => null);
			const statsRes = await labApi.getAttendanceStats({}).catch(() => null);
	...
	const handleCheckIn = async () => {
		...
		try {
			await labApi.markAttendance(payload);  // POST
			await fetchData();
		} catch (e) {
	...
	const handleCheckOut = async () => {
		...
		try {
			await labApi.updateAttendance(payload);  // PUT
			await fetchData();
		} catch (e) {
