...
import { Mail, Lock, Eye, EyeOff, Activity, Phone, Stethoscope } from "lucide-react";
...
export default function DoctorLoginPage() {
  ...
  if (user && !loggingOut) {
    if (["END", "GYN", "ANE"].includes(user.role)) return <Navigate to={`/${user.role.toLowerCase()}`} replace />;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!["END", "GYN", "ANE"].includes(loggedInUser.role)) {
        setLoggingOut(true);
        await logout();
        setLoggingOut(false);
  ...
  return (
    <div style={{ display: "flex", height: "100vh", width: "100%", fontFamily: "Inter, sans-serif" }}>
      {/* Left Side - Green Branding for Doctors */}
      <div style={{
        flex: "1",
        background: "linear-gradient(135deg, rgba(16,185,129,0.85) 0%, rgba(6,78,59,0.95) 100%), url('https://images.unsplash.com/photo-1551076805-e1869043e560?q=80&w=2000&auto=format&fit=crop')",
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: "500px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "20px" }}>
            <div style={{ background: "#10b981", borderRadius: "8px", width: "48px", height: "48px", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <h1 style={{ fontSize: "28px", fontWeight: "600", marginBottom: "16px", lineHeight: "1.3" }}>
            Advanced Clinical Workspace
          </h1>
          <p style={{ fontSize: "15px", lineHeight: "1.6", color: "rgba(255,255,255,0.8)" }}>
            A secure environment dedicated to comprehensive patient care, diagnostics, treatment planning, and surgical management.
          </p>
        </div>
      </div>
      ...
      <div style={{ flex: "1", background: "white", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: "40px", overflowY: "auto" }}>
        <div style={{ width: "100%", maxWidth: "400px" }}>
          {!isSignUp && (
            <div style={{ background: "#ecfdf5", color: "#10b981", padding: "4px 12px", borderRadius: "20px", fontSize: "13px", fontWeight: "500", display: "inline-block", marginBottom: "24px" }}>
              Welcome Doctor
            </div>
          )}

          <h2 style={{ fontSize: "28px", fontWeight: "600", color: "#0f172a", margin: "0 0 8px 0" }}>{isSignUp ? "Sign up" : "Sign in"}</h2>
          <p style={{ color: "#64748b", fontSize: "14px", margin: "0 0 32px 0" }}>Access your clinical administration workspace</p>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                ...
                {!isSignUp && <a href="#" style={{ fontSize: "13px", color: "#10b981", textDecoration: "none", fontWeight: "500" }}>Forgot password?</a>}
              </div>
              <div style={{ position: "relative" }}>
                <div style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", color: "#94a3b8", display: "flex" }}>
                  <Lock size={18} />
            ...
            <button
              type="submit"
              disabled={submitting}
              style={{ width: "100%", padding: "12px", background: "#10b981", color: "white", border: "none", borderRadius: "8px", fontSize: "16px", fontWeight: "500", cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? 0.7 : 1, transition: "background 0.2s" }}
            >
          ...
          <div style={{ marginTop: "40px", borderTop: "1px solid #e2e8f0", paddingTop: "24px", textAlign: "center" }}>
            {isSignUp ? (
              <p style={{ fontSize: "14px", color: "#64748b", margin: 0 }}>
                Already have an account? <button onClick={() => { setIsSignUp(false); setError(""); }} style={{ color: "#10b981", background: "none", border: "none", padding: 0, cursor: "pointer", fontSize: "14px", textDecoration: "none" }}>Sign in</button>
              </p>
            ) : (
              <>
