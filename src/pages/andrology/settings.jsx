...
import { AndrologyApi } from "../../api/andrologyApi";
import "./settings.css";
...
export default function AndrologySettings() {
	...
	// date_joined); profile carries the Andrology-specific fields
	...
	const fetchProfile = async () => {
		try {
			setLoading(true);
			setMessage(null);
			const res = await AndrologyApi.getProfile();
	...
	const handleSaveProfile = async (e) => {
		...
		try {
			setSaving(true);
			setMessage(null);
			const res = await AndrologyApi.updateProfile(formData);
			const merged = mergeProfilePayload(res);
	...
	const handleSaveSecurity = async (e) => {
		...
		try {
			...
			await AndrologyApi.changePassword({
	...
	const handleDeleteProfile = async () => {
		try {
			setSaving(true);
			setMessage(null);
			await AndrologyApi.deleteProfile();
			// This is the Andrology portal — logout/redirect must point here,
			// not to the lab-tech portal.
			await logout("andrology/logout/");
			window.location.href = "/andrology-login";
		} catch (err) {
			console.error(err);
						...
						<form onSubmit={handleSaveProfile}>
							<div className="ls-profile-header">
								...
								<div className="ls-header-info">
									...
									<p>
										{profile.role_display || "Andrologist"} • ID: {profile.employee_id || "N/A"}
									</p>
								</div>
							</div>
							...
							<div className="ls-form-grid">
								...
								<div className="ls-form-group">
									...
									<input
										...
										onChange={handleInputChange}
										disabled={saving}
										placeholder="e.g. Senior Andrologist & ICSI Specialist"
									/>
								</div>
