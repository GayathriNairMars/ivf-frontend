...
export default function MyAttendance() {
	...
	const fetchData = async () => {
		setLoading(true);
		try {
			const todayRes = await hrApi.getTodayAttendance().catch(() => null);
			const histRes  = await hrApi.getAttendanceHistory({}).catch(() => null);
			const statsRes = await hrApi.getAttendanceStats({}).catch(() => null);
	...
	const handleCheckIn = async () => {
		...
		try {
			await hrApi.markAttendance(payload);  // POST
			await fetchData();
		} catch (e) {
	...
	const handleCheckOut = async () => {
		...
		try {
			await hrApi.updateAttendance(payload);  // PUT
			await fetchData();
		} catch (e) {
