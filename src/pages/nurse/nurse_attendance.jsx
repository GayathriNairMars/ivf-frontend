...
export default function NurseAttendance() {
	...
	const fetchData = async () => {
		setLoading(true);
		try {
			const todayRes = await nurseApi.getTodayAttendance().catch(() => null);
			const histRes  = await nurseApi.getAttendanceHistory({}).catch(() => null);
			const statsRes = await nurseApi.getAttendanceStats({}).catch(() => null);
	...
	const handleCheckIn = async () => {
		...
		try {
			await nurseApi.markAttendance(payload);  // POST
			await fetchData();
		} catch (e) {
	...
	const handleCheckOut = async () => {
		...
		try {
			await nurseApi.updateAttendance(payload);  // PUT
			await fetchData();
		} catch (e) {
