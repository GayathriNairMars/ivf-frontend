...
import { Mail, Lock, Eye, EyeOff, Phone, HeartPulse } from "lucide-react";
...
export default function NurseLoginPage() {
  ...
  if (user && !loggingOut) {
    if (["NUR"].includes(user.role)) return <Navigate to={`/${user.role.toLowerCase()}`} replace />;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await login(form.email, form.password, "nurse/login/");
      const loggedInUser = result.user;
      
      if (!["NUR"].includes(loggedInUser.role)) {
        setLoggingOut(true);
        await logout();
        setLoggingOut(false);
        setError("This portal is for Nurses only.");
        setSubmitting(false);
        return;
      }
      navigate(result.redirectUrl || "/nur", { replace: true });
    } catch (err) {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", width: "100%", fontFamily: "Inter, sans-serif" }}>
      {/* Left Side - Pink Branding for Nurses */}
      <div style={{
        flex: "1",
        background: "linear-gradient(135deg, rgba(236,72,153,0.85) 0%, rgba(190,24,93,0.95) 100%), url('/nurse_portal_bg.png')",
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: "500px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "20px" }}>
            <div style={{ background: "#ec4899", borderRadius: "8px", width: "48px", height: "48px", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <p style={{ fontSize: "15px", lineHeight: "1.6", color: "rgba(255,255,255,0.8)" }}>
            A dedicated workspace for nursing staff to manage patient care, monitor vitals, administer treatments, and coordinate with the clinical team.
          </p>
        </div>
      </div>
      ...
      <div style={{ flex: "1", background: "white", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: "40px", overflowY: "auto" }}>
        <div style={{ width: "100%", maxWidth: "400px" }}>
          {!isSignUp && (
            <div style={{ background: "#fdf2f8", color: "#ec4899", padding: "4px 12px", borderRadius: "20px", fontSize: "13px", fontWeight: "500", display: "inline-block", marginBottom: "24px" }}>
              Welcome Nurse
            </div>
          )}

          <h2 style={{ fontSize: "28px", fontWeight: "600", color: "#0f172a", margin: "0 0 8px 0" }}>{isSignUp ? "Sign up" : "Sign in"}</h2>
          <p style={{ color: "#64748b", fontSize: "14px", margin: "0 0 32px 0" }}>Access your nursing administration workspace</p>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                ...
                {!isSignUp && <a href="#" style={{ fontSize: "13px", color: "#ec4899", textDecoration: "none", fontWeight: "500" }}>Forgot password?</a>}
              </div>
              <div style={{ position: "relative" }}>
                <div style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", color: "#94a3b8", display: "flex" }}>
                  <Lock size={18} />
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "32px" }}>
              <input type="checkbox" id="keepSigned" style={{ width: "16px", height: "16px", borderRadius: "4px", border: "1px solid #cbd5e1", cursor: "pointer", accentColor: "#ec4899" }} />
            ...
            <button
              type="submit"
              disabled={submitting}
              style={{ width: "100%", padding: "12px", background: "#ec4899", color: "white", border: "none", borderRadius: "8px", fontSize: "16px", fontWeight: "500", cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? 0.7 : 1, transition: "background 0.2s" }}
            >
          ...
          <div style={{ marginTop: "40px", borderTop: "1px solid #e2e8f0", paddingTop: "24px", textAlign: "center" }}>
            {isSignUp ? (
              <p style={{ fontSize: "14px", color: "#64748b", margin: 0 }}>
                Already have an account? <button onClick={() => { setIsSignUp(false); setError(""); }} style={{ color: "#ec4899", background: "none", border: "none", padding: 0, cursor: "pointer", fontSize: "14px", textDecoration: "none" }}>Sign in</button>
              </p>
            ) : (
              <>
                <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", marginTop: "12px", gap: "16px" }}>
                  <p style={{ fontSize: "14px", color: "#64748b", margin: 0 }}>
                    Are you a Staff? <a href="/login" style={{ color: "#ec4899", textDecoration: "none", fontWeight: "500" }}>Staff login →</a>
                  </p>
                  <p style={{ fontSize: "14px", color: "#64748b", margin: 0 }}>
                    Are you an Admin? <a href="/admin-login" style={{ color: "#ec4899", textDecoration: "none", fontWeight: "500" }}>Admin login →</a>
                  </p>
                  <p style={{ fontSize: "14px", color: "#64748b", margin: 0 }}>
                    Are you a Doctor? <a href="/doctor-login" style={{ color: "#ec4899", textDecoration: "none", fontWeight: "500" }}>Doctor login →</a>
                  </p>
                </div>
              </>
            )}
